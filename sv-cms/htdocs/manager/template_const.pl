...
if ( auth->new() eq 'False' ){	
	print header(-status=>401);
	print "401 Unauthoiruzed!\n";
	exit;
}
...
my $sth=$dbh->prepare("SELECT template_id from domain where domain=?");
$sth->execute($host);
my $template_id=$sth->fetchrow;
my $project_id=$manager->{project_id};
my $domain=$host;
#print Dumper($manager);
#my ($template_id,$project_id,$domain)=$sth->fetchrow();
unless($template_id){
...
if($action=~m{^(update|delfike)$}){
	read_conf::clear_nginx_cache($dbh,$domain);
}
	...
	foreach my $f (@{$params->{FIELDS}}){
		...
		if(($value && $f->{type} eq 'file') || $f->{type} ne 'file'){
			if($f->{type} eq 'file' && $value){
				if($const_val->{$f->{name}}=~m/^([a-zA-Z0-9_\-\.]+)\.([a-zA-Z0-9]+)$/i){
					my $ext=$2;
