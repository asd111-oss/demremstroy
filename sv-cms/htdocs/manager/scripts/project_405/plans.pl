...
use lib '/var/www/clients_sites/kubgs/htdocs/modules';
use PV::dbase;
use PV::tpl;
...
my $t = tpl->new('/var/www/sv-cms/htdocs/manager/scripts/project_405/');

my $img_root_path = '/var/www/sv-cms/htdocs/files/project_405/plans';
...
if(param('plan_add') && $fh){
	...
	if ($ext eq 'gif' || $ext eq 'jpeg' || $ext eq 'jpg' || $ext eq 'png') {
		...
		print FL while (<$fh>);
		close (FL);
		$dbh->query("delete from struct_405_plans where parent_id=? and floor_number=?", $parent_id, $floor_number);
		$dbh->query("INSERT struct_405_plans SET parent_id=?, floor_number=?, photo=?", $parent_id, $floor_number, $filename);
	}

	$t->redirect("./plans.pl?parent_id=$parent_id");
} elsif (param('plan_del') =~ /^(\d+)$/) {
	my $plan = $dbh->select_one("SELECT * FROM struct_405_plans WHERE id=?", $1);

	if (exists $plan->{id}) {
		$dbh->query("DELETE FROM struct_405_plans WHERE id=?", $1);
...
my $floors = $dbh->select_one("SELECT f.header
	FROM struct_405_floors f
	LEFT JOIN struct_405_sections sl ON sl.floor_id=f.id
	WHERE sl.id=?
", $parent_id);

my ($plans) = $dbh->select_all("SELECT * FROM struct_405_plans WHERE parent_id=?", $parent_id);
my %plan = ();
