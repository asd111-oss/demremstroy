...
use lib '/www/sv-cms/htdocs/manager/scripts/project_1136/modules';
use PV::dbase;
use PV::tpl;
...
my $t = tpl->new('/www/sv-cms/htdocs/manager/scripts/project_1136/');

my $img_root_path = '/www/sv-cms/htdocs/files/project_1136/plans';
...
if(param('plan_add') && $fh){
	...
	if ($ext eq 'gif' || $ext eq 'jpeg' || $ext eq 'jpg' || $ext eq 'png') {
		...
		print FL while (<$fh>);
		close (FL);
		$dbh->query("delete from struct_1136_plans where parent_id=? and floor_number=?", $parent_id, $floor_number);
		$dbh->query("INSERT struct_1136_plans SET parent_id=?, floor_number=?, photo=?", $parent_id, $floor_number, $filename);
	}

	$t->redirect("./plans.pl?parent_id=$parent_id");
} elsif (param('plan_del') =~ /^(\d+)$/) {
	my $plan = $dbh->select_one("SELECT * FROM struct_1136_plans WHERE id=?", $1);

	if (exists $plan->{id}) {
		$dbh->query("DELETE FROM struct_1136_plans WHERE id=?", $1);
...
my $floors = $dbh->select_one("SELECT f.header
	FROM struct_1136_floors f
	LEFT JOIN struct_1136_sections sl ON sl.floor_id=f.id
	WHERE sl.id=?
", $parent_id);

my ($plans) = $dbh->select_all("SELECT * FROM struct_1136_plans WHERE parent_id=?", $parent_id);
my %plan = ();
