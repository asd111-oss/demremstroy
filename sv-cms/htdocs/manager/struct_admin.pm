...
use MIME::Base64;
use Data::Dumper;
my $site = '['.$ENV{SERVER_NAME}.'][struct_admin]';
...
sub gen_field{
	...
	my $sth;
	my $id=$form->{id};
	if($element->{before_code} && !$element->{before_code_completed}){
	    if(ref($element->{before_code}) eq 'CODE'){
	      &{$element->{before_code}}($element) ;
	    }
	    else{
	      my $e=$element;
	      #&::pre($e);
	    ...
	    $element->{before_code_completed}=1;
	}
...
#					print "$sth_query";
	...
	elsif($element->{type} eq 'tmpleditor'){
          $field=qq{
		<link rel="stylesheet" href="/manager/javascript/codemirror/lib/codemirror.css"/>
		<script src="/manager/javascript/codemirror/lib/codemirror.js"></script>
		<textarea name='$element->{name}' id='$element->{name}'>$element->{value}</textarea>
		<script> vat t = CodeMirror(document.getElementById('$element->{name}'));</script>
	  };
	}
...
sub get_ev_attr{
	...
	if($element->{read_only} || $element->{readonly}){
		...
		if($element->{placeholder}){
			$element->{placeholder}=~s{"}{&quot;}gs;
			$ev=qq{ placeholder="$element->{placeholder}"}
		}
		return $ev
	}
}
