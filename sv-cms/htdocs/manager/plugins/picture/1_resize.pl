...
# Новые параметры, используються для ресайза, и кропа...
    ...
    wmdeg    => $output_object->{wmdeg}, # Угол поворота текста
    wmfsize  => $output_object->{wmfsize},
    wmfcolor1=> $output_object->{wmfcolor1}, # Цвет 1
    wmfcolor2=> $output_object->{wmfcolor2}, # Цвет 2
...
#    print "Ресайз: не нужен, исходник меньше задоного\t" if ( $opt->{orig} == 1 );
  }
...
#      print "G:$opt->{wmpos} X:$opt->{wmx} Y:$opt->{wmy}\n";
      if ( $w > $wm_width ) {
        ...
        $img->Composite( image   => $watermark,
                         ...
                         x       => $opt->{wmx},
                         y       => $opt->{wmy} );
#        print "Ресайзим водный знак: $wm_width x $wm_height => $opt->{wmresize}\t";
      }
#      print "Добавляем водный знак\t";
  ...
  # Добавляем фильтры...
  if ( $opt->{quality} ){
    $img->Set(quality=>$opt->{quality});
  }
  # Пишем
  $pic = $img->Write( filename => $opt->{file}, quality => ( $opt->{quality} ? $opt->{quality} : 75 ) );
...
#  print "GEOM: $geom";
  ...
  # Пишем текст, с прозрачностью 30%
  $img->Annotate(
    text => $text,
    geometry => $geom, #"+50+50",
