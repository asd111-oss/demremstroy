...
if($action eq 'edit'){
	...
	if($parent_id){ # При добавлении дочернего элемента перегружаем только дочернюю ветку
		print qq{!1<script>var base=window.parent.opener; base.document.getElementById('$sys->{field_name}_sub$parent_id').innerHTML=base.loadDocAsync('./load_1_to_m.pl?config=$sys->{config}&field=$sys->{field_name}&id=$sys->{key}&parent_id=$parent_id'); parent.window.close()</script>};	
	}
	else{ # перегружаем весь 1_to_m
		print qq{!2<script>var base=window.parent.opener; base.document.getElementById('1_to_m_$sys->{field_name}').innerHTML=base.loadDocAsync('./load_1_to_m.pl?config=$sys->{config}&field=$sys->{field_name}&id=$sys->{key}'); parent.window.close()</script>};
	}
	exit;
