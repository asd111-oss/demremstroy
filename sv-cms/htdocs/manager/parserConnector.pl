        ...
        if($orig_filename=~m/([^.]+)$/){
           ...
           my $filename='';
            ...
            $filename.='.'.$ext;
                ...
                open F,qq{>/var/www/sv-cms/htdocs/manager/parser_scripts/tmp/$filename};
#		 open F,qq{>$CMSpath/manager/parser_scripts/tmp/$filename};
        ...
        $info->{full_path}  = qq{/var/www/sv-cms/htdocs/manager/parser_scripts/tmp/$filename};
...
sub render {
    ...
    my $in = shift;
     ...
     eval(q{ 
        ...
        my $f = 'uploadform.tmpl';
        if ( -e "./parser_scripts/template/$in->{project_id}.tmpl" ){
          $f = $in->{project_id}.'.tmpl';
        }
        $template -> process($f, $params->{TMPL_VARS} ) || croak "output::add_template: template error: ".$template->error();
        });
