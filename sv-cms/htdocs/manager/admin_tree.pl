...
if(!$form->{header_field} || $form->{default_find_filter} eq 'header'){
		...
		while($pathstr=~m|\/(\d+)|g){
			my $id=$1;
				if($id){
					 ...
					 $header='__нет_названия__' unless($header=~m{\S});
					 
					 $path.=" / <a href='javascript: show_razdel($id)'>".$header."</a>";
					 $sth->finish();
					 undef($where);
				}
...
#print "Content-type: text/html\n\n";
	while( my $item=$sth->fetchrow_hashref()){
		...
		my $edit_button=qq{<a href="./edit_form.pl?action=edit&config=$form->{config}&id=$id" target="_blank"><img src='/icon/edit.gif'></a>};
