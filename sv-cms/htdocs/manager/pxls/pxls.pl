...
#
# Не выключайте strict, иначе потом сами будете править данный скрипт
#
use strict;
...
if( $Spreadsheet::ParseExcel::VERSION <= 0.57 )
{
	print "Conent-type: text/html\n\nInvalid Spreadsheet::ParseExcel version\n";
	exit;
}

my $curtime=time;
#open CURLOG, ">/tmp/pxls/logs/$curtime.log";
open CURLOG, ">/var/www/sv-cms/htdocs/manager/pxls/logs/$curtime.log";
...
#=======================
use vars qw($config $config_id $project_id);

#=======================
my $USER_DATA=$dbh->selectrow_hashref(
	"SELECT project_id,manager_id from manager where login=?",
	undef,
	$ENV{REMOTE_USER});
...
my $PROJECT_GROUP_SITE=$dbh->selectrow_hashref(
	"SELECT * ".
	"FROM project p, domain d, template t ".
	"WHERE p.project_id=? && p.project_id=d.project_id && d.template_id=t.template_id && t.type=3",
	undef,
...
#=======================
if($PROJECT_GROUP_SITE)		# Для типового проекта
{
	do 'psg.conf';
	if($config)
	{
		$$config{key}="project$$USER_DATA{project_id}";
		$$config{use_project_id}=$$USER_DATA{project_id};
	}
}

else						# Для тех, у кого есть свой конфиг
{
	...
	$sth->execute($config_id, $manager->{project_id});
...
foreach my $e (qw(item tree))
{
	...
	foreach my $field (@{ $$config{ $e }{fields} })
	{
		...
		$$h{hidden}{ $$field{name} }=$field if $$field{is_hidden};
	}
}
#=======================
...
$tmpl->{form}->{a}=$action;
	...
	if($action eq 'upload_file')
	{
		...
		if( $FILENAME ){
			...
			if($$config{is_choose_rubric})
			{
				my $sql="SELECT ".
						"$$config{tree}{fields_hash}{core}{id}{db_name} id, ".
						"$$config{tree}{fields_hash}{core}{header}{db_name} header, ".
						"$$config{tree}{fields_hash}{core}{path}{db_name} path, ".
						"CONCAT(".
							"$$config{tree}{fields_hash}{core}{path}{db_name}".
							",'/',".
							"$$config{tree}{fields_hash}{core}{id}{db_name}".
							") full_path ".
					"FROM $$config{tree}{table} ".
					($$config{tree}{fields_hash}{core}{parent}{default}?"HAVING full_path REGEXP '/$$config{tree}{fields_hash}{core}{parent}{default}(/|\$)' ":'').
					"WHERE ".
						($$config{use_project_id}?"project_id=$$config{use_project_id}":1).' '.
					"ORDER BY full_path, header";

				#ВЛАСОВ ГАД НИ ОДНОГО ПУТНОГО КОМЕНТА БЛИН.
				#print $sql;
				
					
				$$tmpl{tree_id_list}=$dbh->selectall_arrayref($sql, {Slice=>{}});
	...
	elsif($action eq 'parse')
	{
		...
		my $tree_id=param('tree_id');
		$tree_id=~s/\D+//gs if $tree_id;
		...
		if( (exists $$config{run_scripts}{before_parse}) && (ref($$config{run_scripts}{before_parse}) eq 'ARRAY') ){
			foreach (@{ $$config{run_scripts}{before_parse} }){ `$_` }
		}

		##надоело файлы плодить для минимальной работы с базой при парсинге xls антонов.
		#if( (exists $$config{run_scripts}{before_parse_eval}) && (ref($$config{run_scripts}{before_parse_eval}) eq 'ARRAY') ){
		#	foreach (@{ $$config{run_scripts}{before_parse_eval} }){
		#		eval($_);
		#		
		#		if ($@){
		#		print "died: $@";
		...
		parse_file3({
			...
			row_start	=> $ROW_START,
			parent		=> (
							($$config{is_choose_rubric}?$tree_id:undef)
							|| $$config{tree}{fields_hash}{core}{parent}{default}
						   ),
			config		=> $config,
		});
    ...
    if($ws!~/Spreadsheet/){
        ...
        for my $r ($r_min..$r_max){
            ...
            for my $c ($c_min..$c_max){
                ...
                push(@cval,$val || $val eq '0' ? substr($val,0,100):'' );
            }
        ...
        foreach my $rec (@rval){
            ...
            for(my $i=0; $i<scalar(@$row); $i++){
				...
				# Isavnin Konstantin 25.11.2013 добавлено условие $row->[$i], так как в противном случае встретив ноль, писал &nbsp;
				# my $val=$row->[$i] ? $row->[$i] : '&nbsp;';
				
				my $val=$row->[$i] || $row->[$i] eq '0' ? $row->[$i] : '&nbsp;';
#				my $val=$row->[$i];
...
=cut
	...
	# printf LOG "%s\n", join(',', @{ $$CONST{cols}{header} });
	# print LOG "c_index=$c_index\n";
	# print LOG "min=$r_min, max=$r_max\n";

	while($r_max>$r_min)
	{
		my $jj=undef;

		while( my ($col_name, $col_data)=each %{ $$CONST{cols} } )
		{
			# print LOG  "$col_name, $col_data\n";
			if( ref($col_data) eq 'ARRAY' && $ws->get_cell($r_max, $$col_data[0]) )
			{
				$jj=1;
				$c_index=$$col_data[0];
				last;
	...
	if($r_max>$r_min)
	{
		...
		while( my ($k, $data)=each %$color_set )
		{
			$$data{color}=undef;

			if( my $cell=$ws->get_cell( $$data{r}, $c_index ) )
			{
				$$data{color}=$$cell{Format}->{Fill}->[1];
			}
		}
		...
		if( $$color_set{1}{color} eq $$color_set{last}{color} )
		{
			...
			while($r<$r_max)
			{
				...
				if( my $cur_cell=$$CONST{ws}->get_cell($r,$c_index) )
				{
					$color_cur=$$cur_cell{Format}{Fill}[1];
				}

				# if($color_cur ne $color_1st)
				if( $color_cur ne $$color_set{1}{color} )
				{
					# Найдена подгруппа
					$groups=[] if !$groups;
	...
	print LOG "groups: ", $groups?$#$groups+1:'undef', "\n";
	print LOG "goods: ", $goods?$#$goods+1:'undef', "\n";
	# exit;
...
# Khabusev Phanis [pmk@trade.su] 24.10.12 +
# Если получен параметр delete_before_insert,то перед заливной новых товаров удаляем старые.
sub p3_delete
{
    ...
    my $cmd=
        "DELETE FROM $db_table ".
# Isavnin Konstantin [isavnin@trade.su] 06.06.2013 +
#        "WHERE ".
            ($config->{use_project_id} && ($db_table !~ /^(?:test_)?struc/) ? " WHERE project_id=$project_id" : '');
# 
    print LOG "delete_before_insert \n $cmd\n";
    $dbh->do($cmd);
}
# Khabusev Phanis [pmk@trade.su] 24.10.12 -
...
#		 print "$cmd<br>\n<br>\n";
    ...
    $parent_path||='';
    $parent_path=~s/'//g;
	my $q=	"SELECT MAX($tab_sort) ".
		"FROM $db_table ".
		"WHERE ".
			($tab_parent?"$tab_parent='$parent'":"$tab_parent IS NULL").
			($config->{use_project_id} && ($db_table !~ /^(?:test_)?struc/) ? " && project_id=$project_id" : '');
	my $max_sort=$dbh->selectrow_array($q);
	$max_sort||=0;
	$max_sort++;
...
sub p3_add_point
{
    ...
    foreach my $f (keys %$cols)
	{
        ...
        foreach my $i (@{ $cols->{$f} })
		{
            ...
            print CURLOG "$f = ".$data->[$i]."|$f = ".$point->{$f}."\n";
            $j++;
        }
		...
		if($$config{item}{fields_hash}{unique})
		{
			...
			print CURLOG "$cmd, \[".$$point{$tab_uniq}."\]\n";
