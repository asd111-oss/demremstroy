        ...
        if($orig_filename=~m/([^.]+)$/){
           ...
           my $filename='';
            ...
            $filename.='.'.$ext;
                ...
                open F,qq{>/var/www/sv-cms/htdocs/tmp/$filename};
                    binmode F;
                    print F while(<$orig_filename>);
                    #print while(<$orig_filename>);
                close F;
                    
        $info->{full_path}  = qq{/var/www/sv-cms/htdocs/tmp/$filename};
...
sub render {
    ...
    #my $in = shift;    
     ...
     eval(q{ 
        ...
        $template -> process("pp_uploadform_dev.tmpl", $params->{TMPL_VARS} ) || croak "output::add_template: template error: ".$template->error();
        });
...
sub import {
    ...
    if(param('clear') eq '1'){
      $sql="DELETE FROM promo WHERE project_id = $project_id";
      $dbh->do($sql);
    }
	...
	$i++;
      ...
      $sql="REPLACE INTO promo($columns,project_id) VALUES($values)";
