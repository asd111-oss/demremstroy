...
=cut
		...
		# 30.09.2014 Поменял \d на \w по причине использования не числовово ИД в корзине
...
# Не верно получает значение аттрибута
#				while($attr_string=~m/([^;]+?)=([^;]+?)/g){
					my ($attr_name,$attr_value)=($1,$2); # -- имя атрибута и значения для товара из корзины
#					if ( $ENV{HTTP_HOST} =~ /jenta\.biz/ ){
#					  open FH,'>','/home/isavnin/tmp/jenta.bask';
#					  print FH "AN: $attr_name\nAV: $attr_value\nOAV:$opt->{attr}->{$attr_name}\n\n";
#					  print FH "AS: $attr_string\n";
#    					  close FH;
#                                        }
